...
import sqlite3, os, io, smtplib, hashlib, secrets
...
app = Flask(__name__)
app.secret_key = 'velo-super-secret-key'
...
def init_db():
    ...
    # Add gmail columns if upgrading from old db
    try:
...
def dashboard():
    db = get_db()
    total_revenue     = db.execute("SELECT COALESCE(SUM(total_amount),0) FROM invoices WHERE user_id=? AND status='Paid'",   (uid(),)).fetchone()[0]
    total_outstanding = db.execute("SELECT COALESCE(SUM(total_amount),0) FROM invoices WHERE user_id=? AND status='Unpaid'", (uid(),)).fetchone()[0]
    total_invoices    = db.execute("SELECT COUNT(*) FROM invoices WHERE user_id=?", (uid(),)).fetchone()[0]
    ...
    recent = db.execute('SELECT i.*,c.name FROM invoices i JOIN clients c ON i.client_id=c.id WHERE i.user_id=? ORDER BY i.date DESC LIMIT 5', (uid(),)).fetchall()
...
def clients():
    rows = get_db().execute(
        'SELECT c.*, COUNT(i.id) as invoice_count, COALESCE(SUM(i.total_amount),0) as total_billed '
        'FROM clients c LEFT JOIN invoices i ON c.id=i.client_id AND i.user_id=? '
...
def edit_client(client_id):
    db = get_db()
    client = db.execute('SELECT * FROM clients WHERE id=? AND user_id=?', (client_id, uid())).fetchone()
    if not client:
        return redirect(url_for('clients'))
    if request.method == 'POST':
...
def delete_client(client_id):
    db = get_db()
    db.execute('DELETE FROM clients WHERE id=? AND user_id=?', (client_id, uid()))
    db.commit()
...
def invoices():
    ...
    q  = 'SELECT i.*,c.name as client_name FROM invoices i JOIN clients c ON i.client_id=c.id WHERE i.user_id=?'
    p  = [uid()]
    if sf != 'all':
        q += ' AND i.status=?'
        p.append(sf)
    rows = get_db().execute(q + ' ORDER BY i.date DESC', p).fetchall()
...
def create_invoice():
    db = get_db()
    client_list = db.execute('SELECT * FROM clients WHERE user_id=? ORDER BY name', (uid(),)).fetchall()
    if request.method == 'POST':
        count   = db.execute('SELECT COUNT(*) FROM invoices WHERE user_id=?', (uid(),)).fetchone()[0]
        inv_num = f"INV-{count + 101}"
        db.execute("INSERT INTO invoices (user_id,invoice_num,client_id,total_amount,status,date,due_date,notes) VALUES (?,?,?,0,'Unpaid',?,?,?)",
                   (uid(), inv_num, int(request.form['client_id']),
                    datetime.now().strftime('%Y-%m-%d'),
                    request.form.get('due_date',''), request.form.get('notes','')))
        db.commit()
        inv = db.execute('SELECT id FROM invoices WHERE user_id=? AND invoice_num=?', (uid(), inv_num)).fetchone()
...
def invoice_detail(invoice_id):
    ...
    inv = db.execute(
        ...
        'FROM invoices i JOIN clients c ON i.client_id=c.id WHERE i.id=? AND i.user_id=?',
...
def add_item(invoice_id):
    ...
    db.execute('INSERT INTO invoice_items (invoice_id,description,quantity,price,line_total) VALUES (?,?,?,?,?)',
               (invoice_id, request.form['description'], qty, price, qty*price))
    total = db.execute('SELECT COALESCE(SUM(line_total),0) FROM invoice_items WHERE invoice_id=?', (invoice_id,)).fetchone()[0]
    db.execute('UPDATE invoices SET total_amount=? WHERE id=? AND user_id=?', (total, invoice_id, uid()))
    db.commit()
...
def delete_item(invoice_id, item_id):
    db = get_db()
    db.execute('DELETE FROM invoice_items WHERE id=? AND invoice_id=?', (item_id, invoice_id))
    total = db.execute('SELECT COALESCE(SUM(line_total),0) FROM invoice_items WHERE invoice_id=?', (invoice_id,)).fetchone()[0]
    db.execute('UPDATE invoices SET total_amount=? WHERE id=? AND user_id=?', (total, invoice_id, uid()))
    db.commit()
...
def mark_paid(invoice_id):
    db = get_db()
    db.execute("UPDATE invoices SET status='Paid' WHERE id=? AND user_id=?", (invoice_id, uid()))
    db.commit()
...
def mark_unpaid(invoice_id):
    db = get_db()
    db.execute("UPDATE invoices SET status='Unpaid' WHERE id=? AND user_id=?", (invoice_id, uid()))
    db.commit()
...
def delete_invoice(invoice_id):
    ...
    db.execute('DELETE FROM invoices WHERE id=? AND user_id=?', (invoice_id, uid()))
    db.commit()
...
def send_email(invoice_id):
    ...
    try:
        ...
        flash('Gmail auth failed. Check your App Password in Settings.', 'error')
